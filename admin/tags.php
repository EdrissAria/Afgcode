...
if(empty($tag)){
    header("location: tags.php?fill=true");
}elseif(strlen($tag)>20){
    header("location: tags.php?longc=true");
}else{
	if($position == "Subscriber"){
		header("location: tags.php?permission=failed");
	}
    else{
    $record = mysqli_query($link, "INSERT INTO tags (tag,creator,createAt) VALUES ('$tag','$admin','$datetime');");    
    if($record){
        header("location: tags.php?addc=success");
    }else{
        header("location: tags.php?notaddc=true");
    }
}
}
}
              ...
              <nav>
				...
				<div class="row logo-place">
					...
					<li><a href="Admin_panel.php">Dashboard</a></li>
                        ...
                        <li><a href="tags.php" class="active">Add New Tag</a></li>
				...
				<div class="col span-9-of-12">
					...
					<?php if(isset($_GET["fill"])): ?>
			                ...
			                <?php if(isset($_GET["Longc"])):  ?>
			                     <div id="danger" class="animated shake">Too long name for tag!</div>
			                <?php endif; ?>
			                <?php if(isset($_GET["addc"])):  ?>
			                     <div id="success" class="animated pulse">tag is added successfully!</div>
			                <?php endif; ?>
			                <?php if(isset($_GET["notaddc"])):  ?>
			                     <div id="danger" class="animated shake">Can not add tag!</div>
			                <?php endif; ?>
			                <?php if(isset($_GET["deletec"])):  ?>
			                     <div id="success" class="animated pulse">tag deleted successfully</div>
			                <?php endif; ?>
			                <?php if(isset($_GET["notdeletec"])):  ?>
			                     <div id="danger" class="animated shake">Can not delete tag!</div>
			                <?php endif; ?>
							<?php if(isset($_GET["permission"])):  ?>
			                     <div id="danger" class="animated shake">You don't have this permission!</div>
			                <?php endif; ?>
						<label for="tag">Add tag</label>
						<input type="text" name="tag" placeholder="Catagory" id="tag">
						<input type="submit" name="submit" id="button" value="Add New tags">
					</form>
					<table>
						<thead>
							<tr>
						...
						<tbody>
							<?php 
                             	$showCatagory = mysqli_query($link, "SELECT * FROM tags ORDER BY createAt;");
                             	$no = 0;
						    ...
						    <tr>
								...
								<td><?=$datetime;?></td>
								<td><?=$creator;?></td>
								<td><a href="deletetag.php?id=<?php echo $tid;?>">Delete</a></td>
							</tr>
						<?php }?>
						</tbody>
					</table>
